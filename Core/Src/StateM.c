...
void StateM_Dispatch(StateM_signal_t* e)
{
    StateM_state_t sourse, target;

    if (*e < STATEM_SIGNAL_MAX_SIGNALS && StateM.isInitDone == true)
    {
        // Get the source and target states
        sourse = StateM.appState;
        target = StateM.state_table[sourse * STATEM_SIGNAL_MAX_SIGNALS + *e];
        ...
        if (target != STATEM_STATE_NO_STATE && target != sourse)
        {
            ...
            StateM_LRunActionList(StateM.transitions_AL_table[sourse * STATEM_SIGNAL_MAX_SIGNALS + *e]);
